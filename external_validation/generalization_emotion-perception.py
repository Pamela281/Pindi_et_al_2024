...
def load_event_ds000108(dataset, sub, onset, func_ds000108, confound_ds000108, data_path, 
    ...
    conditions_ds000108 = onsets_tr[onsets_tr['trial_type'].isin(['Look_Neutral_Stim', 'Look_Neg_Stim'])]
...
# load csv event Nashiro et al., 2021
def load_event_ds003548(dataset, sub, onset, func_ds003548, confound_ds003548, data_path, 
                        func_files_ds003548, onset_list_ds003548, confounds_ds003548):
    
    data = pd.read_csv(main_path + dataset + '/rawdata/' + onset, sep = '\t')
    
    onsets_tr_a = data[['trial_type', 'onset', 'duration']]
    onsets_tr = onsets_tr_a.copy()
    cond_names = ['neutral', 'sad', 'angry']
    
    conditions_ds003548 = onsets_tr[onsets_tr['trial_type'].isin(cond_names)]

    func_path = data_path + 'func/'
    fmri_data = image.smooth_img(func_path + func_ds003548, fwhm = 6) #☺for the run 1
    
    #load confounds
    confound = pd.read_csv(func_path + confound_ds003548, sep = '\t')
    reg_names = ['trans_x', 'trans_y', 'trans_z', 'rot_x', 'rot_y', 'rot_z']
    confound = confound[reg_names]

    func_files_ds003548.append(fmri_data)
    onset_list_ds003548.append(conditions_ds003548)
    confounds_ds003548.append(confound)
    
    # conditions = conditions.trial_type.unique() 
    
    return func_files_ds003548, onset_list_ds003548, confounds_ds003548, conditions_ds003548   
...
def load_event_ds003823(dataset, sub, func_ds003823, confound_ds003823, data_path, func_files_ds003823, 
    ...
    conditions_ds003823 = onsets_tr[onsets_tr['trial_type'].isin(['vn', 'vt'])]#create a new dataframe with only the condition of interest
...
# first level analysis Nashiro et al., 2021
def first_level_ds003548(func_files_ds003548, onset_list_ds003548, confounds_ds003548):
    TR = 2
    
    glm_ds003548 = FirstLevelModel(
         t_r=TR,
         mask_img = mask_name,
         high_pass=0.008,
         smoothing_fwhm=None)
    ...
    first_level_model = glm_ds003548.fit(func_files_ds003548,events=onset_list_ds003548, 
                                confounds = confounds_ds003548) #add confounds
    
    # design_matrix = first_level_model.design_matrices_[0]
    design_matrices_ds003548 = first_level_model.design_matrices_
    
    return design_matrices_ds003548, glm_ds003548
...
# contrasts Nashiro et al., 2021
def make_localizer_contrasts_ds003548(design_matrix):
       contrast_matrix = np.eye(design_matrix.shape[1])
       contrasts_ds003548 = {
              column: contrast_matrix[i]
              for i, column in enumerate(design_matrix.columns)
       ...
       contrasts_ds003548["LookNeg"] = (
            contrasts_ds003548[" sad"]
            + contrasts_ds003548["angry"])
       
       contrasts_ds003548["LookNeu"] = (
           contrasts_ds003548["neutral"])
       
       # Short dictionary of more relevant contrasts
       contrasts_ds003548 = {
           # "neg - neu": contrasts_ds003548["LookNeg"] - contrasts_ds003548["LookNeu"],
           # "neu - neg": contrasts_ds003548["LookNeu"] - contrasts_ds003548["LookNeg"],
           "neu" : contrasts_ds003548["LookNeu"],
           "neg" : contrasts_ds003548["LookNeg"]}
       
       return contrasts_ds003548
...
def plot_contrast_ds003548(glm, contrast_val, contrast_id, nb_run):
    """Specify, estimate and plot the main contrasts \
        for given a first model."""

    # compute the per-contrast z-map
    summary_statistics_run_ds003548 = glm.compute_contrast(
        contrast_val, output_type="all"
    )
   
    return summary_statistics_run_ds003548
...
for mask in masks:
        ...
        if sub in sub_outliers or sub == 'sub-7007':
            ...
            for i in range(0,len(design_matrices_ds003823)):
                ...
                for index, (contrast_id, contrast_val) in enumerate(contrasts_ds003823.items()):
                    ...
                    if contrast_id == 'neg' or contrast_id == 'neu':
    ...
    #%%####### TEST DS003548 ---------------------------------------
    print('Test... ds003548')
    for i in range(1,len(sub_file_ds003548)+1):
        if i == 2:
            continue
        ...
        func_file_ds003548, onset_file_ds003548, confound_file_ds003548, data_path_ds003548 = load_data_ds003548(sub, derivatives_ds003548, dataset_ds003548)
        
        for k in range(1,len(onset_file_ds003548)+1): #+1 car le script ne faisait pas tous les runs
            run = '%d' % k
            print('Run:' + run)
            
            onset_ds003548 = onset_file_ds003548[int(run)-1]
            func_ds003548 = func_file_ds003548[int(run)-1]
            confound_ds003548 = confound_file_ds003548[int(run)-1]
                
            func_files_ds003548, onset_list_ds003548, confounds_ds003548, conditions_ds003548 = load_event_ds003548(dataset_ds003548, sub, onset_ds003548, func_ds003548, 
                                                                                                                    confound_ds003548, data_path_ds003548, func_files_ds003548, 
                                                                                                                    onset_list_ds003548, confounds_ds003548)
                            
        design_matrices_ds003548, glm_ds003548 = first_level_ds003548(func_files_ds003548, onset_list_ds003548, confounds_ds003548)

        print('Step : save beta-maps')
        for i in range(0,len(design_matrices_ds003548)):
        
             # Call the contrast specification within the function    
            contrasts_ds003548 = make_localizer_contrasts_ds003548(design_matrices_ds003548[i])
            fmri_glm = glm_ds003548.fit(func_files_ds003548[i], design_matrices=design_matrices_ds003548[i])
            
            for index, (contrast_id, contrast_val) in enumerate(contrasts_ds003548.items()):
                
                if contrast_id == 'neg' or contrast_id == 'neu':
            

                    summary_statistics_run_ds003548 = plot_contrast_ds003548(fmri_glm, contrast_val,
                                                           contrast_id, i)
                    
                    contrast_imgs_ds003548.append(summary_statistics_run_ds003548['z_score'])
                    conditions_label_bmaps_ds003548.append(contrast_id)
                    sub_label_bmaps_ds003548.append(sub)
    ...
    print('decoder predict ds003548')
    ds003548_prob = decoder_FREM.predict(contrast_imgs_ds003548)
    ...
    file = open('/neurospin/nfbd/Decoding/output/test-ds000108_permutation_score_' + 
           mask_name.rsplit('/', 8)[-1].rsplit('.')[0]+'_negneu.txt', "w")


    # lines = ['Permutation score = ' + str(np.mean(null_cv_scores)), "Mask_name = "+ mask_name.rsplit('/', 8)[-1].rsplit('.')[0] + "\nContrast =" + list(contrasts)[1]]
    lines = ['Permutation score = ' + str(np.mean(np.array(permuted_scores_ds000108))), "P-value_score = " + str(pvalue_ds000108), "Mask_name = "+ mask_name.rsplit('/', 8)[-1].rsplit('.')[0] + "_negneu"]
    ...
    # Test performed with ds000108
    # Permutation test for accuracy
    actual_score_ds003548, permuted_scores_ds003548, pvalue_ds003548 = permutation_test(contrast_imgs_ds003548, conditions_label_bmaps_ds003548, decoder_FREM_permut_ds003823, sub_label_bmaps_ds003548, n_permutations=1000)
    
    # Opening a file named "permutation_score.txt" in write mode
    print('save permutation score')
    file = open('/neurospin/nfbd/Decoding/output/test-ds003548_permutation_score_' + 
           mask_name.rsplit('/', 8)[-1].rsplit('.')[0]+'_negneu.txt', "w")


    # lines = ['Permutation score = ' + str(np.mean(null_cv_scores)), "Mask_name = "+ mask_name.rsplit('/', 8)[-1].rsplit('.')[0] + "\nContrast =" + list(contrasts)[1]]
    lines = ['Permutation score = ' + str(np.mean(np.array(permuted_scores_ds003548))), "P-value_score = " + str(pvalue_ds003548), "Mask_name = "+ mask_name.rsplit('/', 8)[-1].rsplit('.')[0] + "_negneu"]
    ...
    cls_report_ds000108 = classification_report(conditions_label_bmaps_ds000108, ds000108_prob, target_names=['neg','neu'],output_dict=True)
    # print(cls_report)
    ...
    # classification report ds003548
    cls_report_ds003548 = classification_report(conditions_label_bmaps_ds003548, ds003548_prob, target_names=['neg','neu'],output_dict=True)
    # print(cls_report)

    df2 = pd.DataFrame(cls_report_ds003548).transpose()
    df2.to_csv('/neurospin/nfbd/Decoding/output/test-ds003548_classification_report_' + mask_name.rsplit('/', 8)[-1].rsplit('.')[0]+'.csv')
