...
def load_event(sub, onset, func, confound, data_path, func_files, onset_list, confounds):
    ...
    conditions = onsets_tr[onsets_tr['trial_type'].isin(['Reapp_Neg_Stim','Look_Neg_Stim'])]
...
def make_localizer_contrasts(design_matrix):
    ...
    contrasts["ReappNeg"] = contrasts["Reapp_Neg_Stim"]
    ...
    contrasts = {
        ...
        "reg" : contrasts["Reapp_Neg_Stim"],
        "neg" : contrasts["Look_Neg_Stim"]}
...
for mask in masks:
    ...
    thresholded_img.to_filename(derivatives + 'nilearn/first_level_analysis/weights_img/' + mask.split('.')[0] + '_reg_weights_thr.nii')

    clusters_tb = get_clusters_table(thresholded_img, 1 - threshold)
    clusters_df = pd.DataFrame(clusters_tb)
    clusters_df.to_csv(derivatives + 'nilearn/first_level_analysis/csvfiles/contrasts_imgs_reapp_vs_neg_ds000108/' + mask.split('.')[0] + '_reg_clusters_table.csv', index=False)
    ...
    df2.to_csv(derivatives + 'nilearn/first_level_analysis/csvfiles/contrasts_imgs_reapp_vs_neg_ds000108/summary_table_data_across_sub_' +
