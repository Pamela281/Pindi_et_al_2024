...
def load_event(sub, data_path, func_files, onset_list, confounds):
    ...
    conditions = onsets_tr[onsets_tr['trial_type'].isin(['vn', 'dn'])]  # Filter conditions of interest
...
for mask in masks:
    ...
    for sub in sub_file:
        ...
        cmap.append(zmap)
        conditions_label_bmaps.append('reg - neg')
    ...
    thresholded_map2.to_filename(
        os.path.join(main_path, dataset, 'derivatives', 'nilearn', 'second_level_analysis', 'contrast_img', 
                     f'{mask_name.rsplit("/", 8)[-1].rsplit(".")[0]}_reg_thresholdedmap.nii')
    )
    ...
    table.to_csv(
        os.path.join(main_path, dataset, 'derivatives', 'nilearn', 'second_level_analysis', 'contrast_img',
                     f'{mask_name.rsplit("/", 8)[-1].rsplit(".")[0]}_regneg_table.csv')
    )
