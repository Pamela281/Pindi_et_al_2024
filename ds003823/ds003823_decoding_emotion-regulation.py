...
def make_localizer_contrasts(design_matrix):
    ...
    contrasts["ReappNeg"] = (contrasts["dn"])
    

    # Short dictionary of more relevant contrasts
    contrasts = {
        "neg - reg": contrasts["LookNeg"] - contrasts["ReappNeg"],
...
for mask in masks:
    ...
    for sub in sub_file:
        ...
        conditions = onsets_tr[onsets_tr['trial_type'].isin(['vn','dn'])]
    ...
    # Save weight image
    weight_img = decoder_FREM.coef_img_['reg']
    weight_img.to_filename(os.path.join(derivatives, 'nilearn', 'first_level_analysis', 'weights_img', f'{mask.split(".")[0]}_reg_weights.nii'))
    ...
    coef_img = decoder_FREM_permut.coef_img_['reg']
    ...
    thresholded_img.to_filename(os.path.join(derivatives, 'nilearn', 'first_level_analysis', 'weights_img', f'{mask.split(".")[0]}_reg_weights_thr.nii'))
    
    # Save clusters table
    clusters_tb = get_clusters_table(thresholded_img, 1 - threshold)
    clusters_df = pd.DataFrame(clusters_tb)
    clusters_df.to_csv(os.path.join(derivatives, 'nilearn', 'first_level_analysis', 'csvfiles', 'contrasts_imgs_reappneg_vs_neg_ds003823', f'{mask.split(".")[0]}_reg_clusters_table.csv'), index=False)
    ...
    summary = {
        'cv_scores': list(decoder_FREM.cv_scores_['reg']),
        'mask': mask.split('.')[0]
    }
    
    df2 = pd.DataFrame.from_records(summary)
    df2.to_csv(os.path.join(derivatives, 'nilearn', 'first_level_analysis', 'csvfiles', 'contrasts_imgs_reappneg_vs_neg_ds003823', f'summary_table_data_across_sub_{mask.split(".")[0]}_noerror.csv'))
    
    # Opening a file named "permutation_score.txt" in write mode
    file = open(derivatives+'nilearn/first_level_analysis/csvfiles/contrasts_imgs_reappneg_vs_neg_ds003823/permutation_score_' + 
